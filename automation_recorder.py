...
from auto_control import click_button, input_text, double_click_button, hide_cursor, show_cursor
...
class AutomationRecorder:
    ...
    def setup_ui(self):
        ...
        self.action_list = tk.Listbox(list_frame)
        ...
        ttk.Button(config_frame, text="保存配置", command=self.save_config).pack(side="left", padx=5)
        ttk.Button(config_frame, text="选择配置", command=self.choose_config).pack(side="left", padx=5)
    ...
    def toggle_recording(self):
        # Linux 上需要 root 权限才能使用 keyboard 模块
        try:
    ...
    def record_position(self, _):
        ...
        dialog.title("选择操作类型")
        dialog.geometry("300x200")  # 增加高度以适应新的选项
    ...
    def start_playback(self):
        ...
        try:
            ...
            for i, action in enumerate(self.actions):
                if not self.is_playing:  # 检查是否应该停止回放
                    break
                    
                try:
                    if action["type"] == "click":
                        click_button(action["x"], action["y"])
                    elif action["type"] == "double_click":
                        double_click_button(action["x"], action["y"])
                    elif action["type"] == "input":
                        input_text(action["x"], action["y"], action["text"])
                    ...
                    if self.is_playing:  # 如果没有被停止，则等待后继续
                        time.sleep(1)
